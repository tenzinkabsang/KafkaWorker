...
public class RegistryJsonConsumerTests(ITestOutputHelper testOutputHelper)
{
    [Fact]
    public async Task ProcessesSuccessfully()
    {
        ...
        await KafkaHelper.InitializeTopicAsync(topic, message, s => new JsonSerializer<OrderMessage>(s).AsSyncOverAsync());
        ...
        await KafkaHelper.PublishMessageAsync(topic, "key", message, s => new JsonSerializer<OrderMessage>(s).AsSyncOverAsync());
    ...
    public async Task InvalidMessage_SentToDlq()
    {
        ...
        await KafkaHelper.InitializeTopicAsync(topic, invalidMessage, s => new JsonSerializer<OrderMessage>(s).AsSyncOverAsync());
        ...
        await logProvider.WaitForLogAsync("Subscribed to kafka", hostTask);
        await Task.Delay(TimeSpan.FromSeconds(3));
        await KafkaHelper.PublishMessageAsync(topic, "key", invalidMessage, s => new JsonSerializer<OrderMessage>(s).AsSyncOverAsync());
